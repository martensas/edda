...
%		\setcounter{footnoteB}{0}% Not currently using these.
	...
	\NewDocumentCommand{\bva}{o o}{% Begin verse a
		...
		\IfValueTF{#1}{%
			\numberstanzafalse% If optional stanza number exists or = 0 we disable the automatic numbering
		}{%
			\numberstanzatrue%
			\Xstanzaonlyfirst% For printing the stanza number in the apparatus
		}%
		\begin{normalsize}\begin{stanza}%
		\IfValueT{#1}{% Optional stanza number
			\IfEq{#1}{0}{}{% If optional stanza number is NOT 0 we show it with flag.
				\flagstanza{\textbf{#1}}%
			}%
		}%
	}
	\NewDocumentCommand{\eva}{o}{% End verse a
		\& \end{stanza}\end{normalsize}% End large and stanza
  ...
  \NewDocumentCommand{\bpa}{o o}{% Begin prose a
    \IfValueT{#2}{% Optional line number to start on (otherwise we start on 1)
      \setlinenum{#2}%
		...
		\begin{normalsize}% Begin large text
		\pstart% Start prose
	...
	\NewDocumentCommand{\epa}{o}{% End prose a
		\pend\end{normalsize}% End large and prose
