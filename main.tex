% NOTE: This file should be compiled with XeLaTeX.

\documentclass[9pt]{memoir}

% Various packages
\usepackage{xparse}% For better document commands
\usepackage{graphicx}% For rotating characters (used when citing runic inscriptions) and images
\graphicspath{ {./img/} }
%NOTE: Continuous figure numbering is after mainmatter
%\usepackage[inline]{showlabels}% For debugging labels

\usepackage{longtable}% Long tables.
\usepackage{tabularx}% Table management

% NOTE: Fixes for the epub version only
\makeatletter
\ifdefined\HCode
  % longtables are very annoying, but since epubs don't have pages we don't need them.

  \renewenvironment{longtable}[2][]{%
    \begin{tabular}{#2}%
  }{%
    \end{tabular}%
  }
  % Neutralize longtable-specific header/footer commands
  \def\endhead{}
  \def\endfirsthead{}
  \def\endfoot{}
  \def\endlastfoot{}

  % Fix PDF-specific link annotation commands
  \def\Hy@pstringdef#1#2{\gdef#1{#2}}
  \def\@pdfm@mark#1{}

  \@ifundefined{c@Hy@AnnotLevel}{\newcounter{Hy@AnnotLevel}}{}
  \def\Hy@BeginAnnot{}
  \def\Hy@EndAnnot{}

  % vibe-coded nonsense
  \usepackage{luacode}
    \begin{luacode*}
      -- 1. Define the line-break function, but do NOT activate it yet
      function reledmac_stanza_br(line)
        if string.match(line, " &%s*$") or string.match(line, " &%s*%%") then
          line = string.gsub(line, " &(%s*)$", "\\HCode{<br />} &%1")
          line = string.gsub(line, " &(%s*%%)", "\\HCode{<br />} &%1")
        end
        return line
      end
    \end{luacode*}

    % 2. Wait until the preamble is completely finished, then attach the listener
    \AtBeginDocument{
      \directlua{luatexbase.add_to_callback('process_input_buffer', reledmac_stanza_br, 'reledmac_stanza_br')}
    }
\fi
\makeatother

\usepackage{rotating}
\usepackage{tipa}% IPA

\PassOptionsToPackage{hyphens}{url}\usepackage[hidelinks=true]{hyperref}% For linking within the pdf

% Critical edition
\usepackage[parapparatus, series={A,B}]{reledmac}
\newcounter{prosea}
\newcounter{versenum}

%\def\critfootnotes{}%NOTE: experimental, if we want critical notes in footnotes

% Font and typesetting
\usepackage[a5paper, %Geometry
  bindingoffset=0.2in,
  left=1in,
  right=1in,
  top=0.9in,
  bottom=0.8in,
  footskip=.25in]{geometry}

% Headers.
\def\booktitle{}
\pagestyle{myheadings}
\aliaspagestyle{plain}{empty}% Remove footers from title pages
\makeoddhead{myheadings}{}{\emph{\booktitle}}{\thepage}
\makeevenhead{myheadings}{\thepage}{The Northern Epics}{}

% Font and colour.

\usepackage[usenames,dvipsnames]{xcolor}
\definecolor{myRed}{RGB}{178, 0, 0}
\definecolor{myBlack}{RGB}{33, 37, 41}

\ifdefined\HCode \else
\usepackage{microtype}
\color{myBlack}
\fi

\usepackage{fontspec}
\usepackage{wasysym}
\setmainfont{Junicode}[Numbers=Lining]

\usepackage{tikz}% For drawing figures
\usetikzlibrary{positioning, calc}

\usepackage{polyglossia}
\setdefaultlanguage{english}
\setotherlanguages{greek, russian, hebrew, latin}
\newfontfamily\greekfont{Junicode}
\newfontfamily\cyrillicfont{Times New Roman}
\newfontfamily\hebrewfont{Ezra SIL}[Script=Hebrew]


% Bibliography and citations
\usepackage{csquotes}
\usepackage[style=authoryear, backend=biber]{biblatex}
\addbibresource{bibliography.bib}
\DefineBibliographyStrings{english}{%
  sequens = {f\adddot},
  sequentes = {ff\adddot},
}

\DeclareCiteCommand{\parenciteshorthand}[\mkbibparens]%
  {\usebibmacro{prenote}}%
  {\usebibmacro{citeindex}%
   \printfield[bibhyperref]{shorthand}}%
  {\multicitedelim}%
  {\usebibmacro{postnote}%
}

\DeclareCiteCommand{\textciteshorthand}%
  {\usebibmacro{prenote}}%
  {\usebibmacro{citeindex}%
   \printfield[bibhyperref]{shorthand}}%
  {\multicitedelim}%
  {\iffieldundef{postnote}{}%
    {\addspace\mkbibparens{\printfield{postnote}}}
  }%


\begin{document}

% language handling

\NewDocumentCommand{\textoi}{m}{%
  \emph{#1}%
}

\let\oldtextlatin\textlatin% NOTE: Wrap textlatin in \emph{}
\renewcommand{\textlatin}[1]{\emph{\oldtextlatin{#1}}}

% Book and chapter commands
  \setsecnumdepth{none}% Disable numbering of sections
  \maxsecnumdepth{none}% Disable numbering of sections

	\NewDocumentCommand{\chapterStart}{o O{Chap}}{% Command at the start of chapter, actually just resets internal counters
		\setcounter{stanza}{0}%
		\setcounter{prosea}{0}%
		\stepcounter{section}%
		\IfNoValueF{#1}{%
			\begin{center}%
			\textbf{#2. \arabic{section}} \\
			{#1}\end{center}%
		}%
	}

  \def\thisBookCode{INTERNALERROR}%

  \NewDocumentCommand{\setBookCode}{m}{
    \def\thisBookCode{#1}%
    \phantomsection\label{#1}%
  }

	\NewDocumentCommand{\bookStart}{m o}{% Command at the start of book
		% arg 1 (mandatory): English title
		% arg 2 (optional):  Original title
	  \IfValueTF{#2}{%
      \chapter*{#1 \\ \emph{(#2)}}%
      \def\booktitle{#2}%
    }{%
      \chapter*{#1}%
      \def\booktitle{#1}%
    }%
    \addcontentsline{toc}{chapter}{\booktitle}
    \setcounter{section}{0}% Set chapter count to zero.
    \chapterStart{}%
	}

\Xlemmaseparator{~|}% Use vertical bar instead of right bracket ] to break lemmas
\Xinplaceofnumber{0pt}% for ALL-notes to work properly

\ifdefined\critfootnotes% Print stanza number
  \Xstanza
  \Xstanzaonlyfirst
  \Xnolinenumberifannotation
  \Xstanzaseparator{/}
\fi

% Verse format commands
	\numberstanzatrue% Use reledmac stanza numbering

  \NewDocumentCommand{\textlink}{m o o}{%
%  NOTE: Link to any poem using the bookcode and stanza number, e.g.
%        \textlink{Vafthrudnismal}[11].
%        #3 is an optional display number
    \IfNoValueTF{#2}{%
      \hyperref[#1]{\csname#1\endcsname}%
    }{%
      \IfNoValueTF{#3}{%
        \hyperref[#1:#2]{\csname#1\endcsname\ #2}%
      }{%
        \hyperref[#1:#2]{\csname#1\endcsname\ #3}%
      }%
    }%
  }

	\NewDocumentCommand{\bvg}{o}{% Begin verse group
    \ifdefined\HCode
        \leavevmode\HCode{<div style="height: 0.3cm;"></div>}%
    \else
        \vspace{0.3cm}%
    \fi
		\ifx\critfootnotes\undefined \begin{ledgroup}\fi%
		\beginnumbering%
	}

	\NewDocumentCommand{\bva}{o o}{% Begin verse a
		\IfValueT{#1}{% Optional stanza number
			\IfEq{#1}{0}{}{% If optional stanza number is NOT 0 we show it instead of expected number
        \renewcommand{\thestanza}{\textbf{#1}}%
			}%
    }%
    \begin{large}\begin{stanza}%
    \IfValueT{#2}{% Optional line number to start on (otherwise we start on 1)
  		\setline{#2}%
  	}%
    \IfValueTF{#1}{% Make labels
      \IfEq{#1}{0}{}{%
        \phantomsection\edef\temp{\noexpand\label{\thisBookCode:#1}}\temp%
      }%
    }{%
      \phantomsection\edef\temp{\noexpand\label{\thisBookCode:\arabic{stanza}}}\temp%
    }%
	}

	\NewDocumentCommand{\eva}{o}{% End verse a
		\&%
    \end{stanza}\end{large}% End large and stanza
    \vspace{1.5mm}% Vertical space
    %
	}

	\NewDocumentCommand{\bvb}{o}{% Begin verse b (see above)
%		\IfNoValueTF{#1}{% NOTE: Not currenly numbering these.
%			\stepcounter{verseb}%
%			\textbf{\arabic{verseb} }%
%		}{\IfEq{#1}{0}{}{%
%			\textbf{\textbf{#1}}
%		}}%
		\noindent%
	}
	\NewDocumentCommand{\evb}{o}{% End verse b
		% Nothing (for now?)
	}

	\NewDocumentCommand{\evg}{o}{% End verse group
		\endnumbering%
		\ifx\critfootnotes\undefined \end{ledgroup}\fi%
		\vspace{0.3cm}%
	}

% Prose format commands

	\NewDocumentCommand{\bpg}{o}{% Begin prose group
		\vspace{0.3cm}%
    \ifx\critfootnotes\undefined \begin{ledgroup}\fi%
		\beginnumbering%
  }

  \NewDocumentCommand{\bpa}{o o}{% Begin prose a
    \setcounter{versenum}{0}% Reset the counter for the verse
		\setlength{\leftskip}{\stanzaindentbase}% Get some indentation to agree with stanza indent
		\begin{large}\pstart% Start large and prose
		\noindent% No indent (is this bad)
    \IfValueT{#2}{% Optional line number to start on (otherwise we start on 1)
      \setline{#2}%
    }%
		\IfNoValueTF{#1}{% Add prose number according to counter
			\stepcounter{prosea}% Step prose counter
      \phantomsection\edef\temp{\noexpand\label{\thisBookCode:P\arabic{prosea}}}\temp%
      \flagstanza{\textbf{P\arabic{prosea}}}%
		}{\IfEq{#1}{0}{}{% If optional prose number is NOT 0 we show it.
      \IfEq{#1}{0}{}{%
        \phantomsection\edef\temp{\noexpand\label{\thisBookCode:P#1}}\temp%
      }%
      \flagstanza{\textbf{P#1}}%
		}}%%
	}

  \NewDocumentCommand{\pva}{o}{% Begin prose verse a
    \IfNoValueTF{#1}{%
      \stepcounter{versenum}}%
      {\setcounter{versenum}{#1}}%
    \textsuperscript{\textbf{\arabic{versenum}}}}

	\NewDocumentCommand{\epa}{o}{% End prose a
		\pend\end{large}% End large and prose
    \numberpstartfalse%
		\setlength{\leftskip}{0cm}%
		\vspace{1.5mm}% Vertical space
	}

	\NewDocumentCommand{\bpb}{o}{% Begin prose b (see above)
    \setcounter{versenum}{0}% Reset the counter for the verse
    \noindent%
	}
  \NewDocumentCommand{\pvb}{o}{% Begin prose verse b
    \IfNoValueTF{#1}{%
      \stepcounter{versenum}}%
      {\setcounter{versenum}{#1}}%
    \textsuperscript{\textbf{\arabic{versenum}}}}
	\NewDocumentCommand{\epb}{o}{% End prose b
		% nothing (for now?)
	}

	\NewDocumentCommand{\epg}{o}{% End verse group
		\endnumbering%
    \ifx\critfootnotes\undefined \end{ledgroup}\fi%
		\vspace{0.3cm}%
	}


% Formatting of footnotes and critical apparatus

	% Edtext with automatic translation
	% EXAMPLE: \edtrans{orðinn}{become}{\Afootnote{borinn `borinn' \Hauksbok}}
	\NewDocumentCommand{\edtrans}{m m +m}{%
		\edtext{#1}{\lemma{\let\alst\relax #1 `#2'}#3}%
	}

  % All note
	\NewDocumentCommand{\Ballnote}{+m}{%
    \ifdefined\critfootnotes%
		  \edtext{}{\linenumannotation{ALL}\Bfootnote{{#1}}}%
    \else%
      \edtext{}{\lemma{ALL}\Bfootnote[nonum]{{#1}}}%
    \fi
	}
  \NewDocumentCommand{\Aallnote}{m}{%
    \ifdefined\critfootnotes%
      \edtext{}{\linenumannotation{ALL}\Afootnote{{#1}}}%
    \else%
      \edtext{}{\lemma{ALL}\Afootnote[nonum]{{#1}}}%
    \fi
  }

	% Manuscript note
	\leftnoteupfalse\rightnoteupfalse% Make the notes flow downwards
	\NewDocumentCommand{\mssnote}{m}{%
    \ifdefined\HCode
      \HCode{<div style="text-align: right; font-size: 0.85em; margin-bottom: 0.5em;">}%
      [\textit{Manuscripts:} #1]%
      \HCode{</div>}%
    \else
      \ledrightnote{[{#1}]}%
    \fi
%   \begingroup
%   \renewcommand{\thefootnoteA}{}%
%   \footnoteA{\emph{Mss.: } #1}%
%   \endgroup%
	}

	\NewDocumentCommand{\speakernote}{m}{%
		\ledleftnote{{#1}}%
	}

  \NewDocumentCommand{\speakernoteb}{m}{%
  %  {\footnotesize #1} \\%
    \leavevmode\llap{%
      \parbox[t]{1.5in}{\raggedleft\footnotesize #1}%
      \hspace{0.1in}%
      }%
  }

	% Sidenote margin
	\setlength{\ledlsnotesep}{1.5 \ledlsnotesep}

	% Make A footnotes paragraphs
	\Xarrangement[A]{paragraph}
	% Make A footnotes italicized
	\Xwrapcontent[A]{\emph}

	% Make numbering of B footnotes roman. Disabled since value cannot be higher than "z".
%	\renewcommand*{\thefootnoteB}{\alph{footnoteB}}

	\mpfnpos{% Footnote order
		{B}{familiar},%
		{A}{familiar},%
		{A}{critical},%
		{B}{critical},%
	}


% Poem formatting
	% First line number at 2
	\firstlinenum{2}
	\linenumincrement{2}

	% Stanza indentation (required by reledmac)
%  \setlength{\stanzaindentbase}{20pt}
	\setstanzaindents{5, 1}
	\setcounter{stanzaindentsrepetition}{1}

  \sublinenumberstyle{alph}

%  \renewcommand{\stanzanumwrapper}[1]{%
%    \flagstanza[35pt]{#1}%
%  }%

	% Line numbers directly under verse number (kind of a hack)
%  \setlength{\linenumsep}{-0.5pc}

	% Mark cæsuræ. Bullet point ( • ), interpunct: ( · )
	\NewDocumentCommand{\hld}{o}{%
    \ifdefined\HCode
      \IfNoValueTF{#1}{ ·}{%
        {\small ({#1})}%
      }%
    \else
      \IfNoValueTF{#1}{ · }{%
        {\small ({#1})}%
      }%
    \fi
  }%

	% Indent lines (in Ljóðaháttr or Galdralag).
	\newcommand{\ind}{%
		\hspace*{1.5em}%
	}

	% Mark alliteration.
	\NewDocumentCommand{\alst}{m}{%
%		\underline{#1}% Underline.
%		\textbf{#1}% Bold font. To be used if printing in black and white.
  	\textcolor{myRed}{#1}% Red font. Uses xcolor package.
	}

% Translation formatting

  \newcommand{\bce}{%
    \textsc{bc}%
  }

  \newcommand{\ce}{%
    \textsc{ad}%
  }

	%Line. NOTE: Commented out for now
	\newcommand{\sectionline}{%
	%	\begin{center}\line(1,0){5em}\end{center}%
	}

	% Mark kennings.
	\NewDocumentCommand{\ken}{sm}{%
		% No small caps; text inside the brackets.
		\IfBooleanTF{#1}{% No small caps
			{[{#2}]}% EXAMPLE: [= Wooden]
		}{%
			\textsc{[{#2}]}% EXAMPLE: [ETTIN]
		}%
	}%

	% Mark names with angular brackets.
	\NewDocumentCommand{\name}{m}{%
	% Text inside the brackets.
		⟨{#1}⟩% EXAMPLE: ⟨ettin⟩
	}%

% Index commands
	\NewDocumentCommand{\inx}{o m O{#2}}{% Index link (type, link, optional alt display)
    \ifdefined\HCode% TODO: Temporary fix for the epub
      {#3}%
    \else
		  \hyperref[#1:#2]{#3}%NOTE \hyperref[#1:#2]{#3\textsuperscript{†}}% NOTE \hyperref[#1:#2]{#3\textsuperscript{#1}
    \fi
	}

	\NewDocumentCommand{\inxitem}{o m}{% Index label (type, word)
		\item[\textbf{#2}]%
		\phantomsection\label{#1:#2}%
	}

% Render books

  \input{books.tex}%

\end{document}
