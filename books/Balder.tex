...
ok \alst{r}agna \alst{r}ǫk \hld\ \edtrans{\alst{r}júfęndr}{rippers}{\Bfootnote{Presumably Surt and Lock with his children, as described in \textlink{Voluspa} 40 ff.  The verb \emph{rjúfa} ‘\CV: to break, rip up, break a hole in’ is used in the same context in the formulaic \emph{þá’s rjúfask ręgin} ‘when the \inx[G]{Reins} are ripped’ (\textlink{Vafthrudnismal} 52), \emph{und’s (of) rjúfask ręgin} ‘until the Reins are ripped’ (\textlink{Grimnismal} 4, \textlink{Lokasenna} 41 and \textlink{Sigrdrifumal} 17).
