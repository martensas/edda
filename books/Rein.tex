...
The \textbf{Speeches of Rein} (\textlink{Reginsmal}) is preserved in \Regius, where it follows \textlink{Gripisspa} and is introduced with a large initial and an illegible title originally in red ink.  \textlink{Reginsmal} clearly serves as the basis for \VolsungaSaga\ 14–15 and 17–18 (for ch. 16 see \textlink{Gripisspa}), where sts. 1, 2, 6, and 18 are cited.

\subsection{The Siward sequence}

In \Regius, \textlink{Reginsmal} is the first of a group of three very similar “poems” in an unbroken narrative sequence, for which reason the whole group will be discussed here.  The group also includes \textlink{Fafnismal} and \textlink{Sigrdrifumal}.

The existence of these three “poems”—indeed their very names—is really a product of later philology, and to paraphrase Bellows, it is doubtful whether it is logically sound.  Although \textlink{Fafnismal} is introduced by a title and large initial and thus separated from \textlink{Reginsmal}, the line between \textlink{Fafnismal} and \textlink{Sigrdrifumal} is entirely arbitrary since the two are continuous in the ms.  More importantly, none of the three “poems” is a unitary composition; but throughout them one finds the same amalgamation of narrative prose and stanzas in alternating \Fornyrdislag\ and \Ljodahattr.  It may be noted that the style of the \Ljodahattr\ stanzas is very similar throughout, and this may also be the case for the \Fornyrdislag-stanzas, so that we may be dealing with at least two long separate cycles treating the same overlapping story.  A particularly transparent example of overlap between sources is the speech of the tits in \textlink{Fafnismal} (TODO: stanza numbers), where there is a good logical progression of thought if one only reads the stanzas in one meter, which is lost if one reads both.

Since they are not three distinct poems (unlike say \textlink{Voluspa}, \textlink{Grimnismal} and \textlink{Vafthrudnismal}), the whole group should be understood as a continuous narrative saw or \emph{prosimetrum}, where the redactor tells the story primarily through prose, with the stanzas are reserved for direct speech.  It is not improbable that this reflects some convention of oral storytelling.  In any case, the old division into three poems has been retained in the present edition for reasons of accessibility, but the reader is encouraged to read the entire sequence in order.
...
\bvb “{\huge W}\textsc{hat sort} of fish is this which runs in the flood? \\
\ind It cannot help itself from harm. \\
Redeem thy head out of Hell; \\
\ind find me the fountain’s flame \ken{gold}!”\evb\evg
