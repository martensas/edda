...
The segmenting of the words is ambiguous.  The pres.~ed.~reads the line as a type E, but it may also be read as a type D \emph{hôh hurnid-skip} ‘high horned-ship’.  To determine the correct reading we may cf.~the semantics of the name of the god Balder’s ship in Norse mythology, \emph{Hring-horni} ‘Ringhorn’, i.e.~‘one with a coiled prow’ (\Gylfaginning\ 49, \Skaldskaparmal\ 12), which ought to be compared with OE \emph{hringid-stęfná} ‘ringed-prowed one’, a term for a ship (\Beowulf\ 32, 1131, 1897; see \cite[116]{KlaeberBeowulf}).  It seems clear from these parallels that the term \emph{horn} is an archaic/poetic term for the prow of a ship, and therefore the pres.~l.~must be read as ‘high-horned ship’, not ‘high horned-ship’, since the important thing is that the prow is high, not that the ship has a prow (a tautology).}}; \hld\ \alst{h}ęliðos kwámun, &
