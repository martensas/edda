...
\bvg\bva[][56]%
Dár ni mak denae \alst{m}ák andremo \hld\ helfan vora demo \edtrans{\alst{M}ú·spille}{the Muspell}{\Bfootnote{The word which has given the poem its modern name, most likely ‘the Destruction of the World’.  \emph{Mú·spille} is a hapax in OHG, but corresponds to OS \emph{Múd·spelli} \char`~\ \emph{Mút·spelli} (\textlink{Heliand}[31][2591], 4358) and ON \emph{Mú·spell} (see \textlink{Voluspa}[49]/1 n.)
In all cases the word seems to refer to an element of the eschatological destruction of the world, whether Heathen (as in \textlink{Voluspa}) or Christian (as in \Muspilli\ and \textlink{Heliand}).  Its exact sense and etymology are hard to pin down, and the following discussion will only give a short outline.

The word is clearly a cpd., segmented between \emph{ú} and \emph{s}; it is a \emph{ja}-stem in OS and OHG and an \emph{a}-stem in ON; its gender is masc.~in ON and neutr.~or masc.~in OS and OHG.
The first element, the root length of which is actually uncertain, has been explained either as < PGmc. \emph{*múh-} ‘heap (of earth)’ or (on the basis of the OS dental) \emph{*munþaz} ‘mouth’.  On the basis of sound-laws the latter is unlikely since \textipa{/*VnT/} > \textipa{/\~*V:T/} (part of the Ingvaeonic nasal spirant law) does not affect OHG or ON, which for ‘mouth’ have \emph{mund} and \emph{munnr}, respectively.
The second element is prob. an agent noun, derived either from the verb \emph{*spilþijaną} ‘to spoil, ruin; kill, murder’ (OE \emph{spillan, spildan}, OS \emph{spildjan}, ON \emph{spilla}) or from \emph{*spellōną} ‘to tell, talk’.  Due to the lack of breaking in ON the former is probably more likely.  Still, due to the possibility of borrowing between Germanic languages, the lack of any gloss or explanation and the formal difficulties, any derivation must be considered uncertain (\textciteshorttitle[s.v. \textit{mu᷋spili}]{EWA}).}}, &
