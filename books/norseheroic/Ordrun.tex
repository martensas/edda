...
\alst{b}auð hann ęnn við mér \hld\ \alst{b}ú \edtext{fimm-t\emph{ía}n}{\Afootnote{\emph{.xv.} \Regius}\lemma{fimm-t\emph{ía}n ‘fifteen’}\Bfootnote{For metrical reasons the abbreviation is resolved to the archaic hiatus form \emph{fimm-tían} rather than the standard \emph{fimm-tán}.
The meter of \Oddrunargratr\ does rarely permit 3-syllable verses (verses 4/1a–b, 4/2b, 6/1a), but restoring hiatus is the better option since it agrees with the typical metrical patterns of the poem; \emph{-tían} is also metrically secured from the early C11th by Þorm \emph{Þorgdr} (\Skp\ 5), 1/4b: \emph{fimm-tían} ‘fifteen’, 15/2b: \emph{þré-tían} ‘thirteen’.
